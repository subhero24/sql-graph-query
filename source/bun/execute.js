...
export default function execute(db, query, parents) {
	...
	if (parents == undefined) {
		if (sql) {
			...
			let entries = db.query(`${sql} RETURNING ${attributeSql}`).all(variables);
	...
	for (let relation of relations) {
		...
		if (relationIsJson) {
			...
			if (query.type == undefined) {
				...
				let key;
				let references = db.query(`PRAGMA foreign_key_list("${resource}")`).all();
			...
			let statement = db.query(`${scope} SELECT ${attributeSql} FROM temp ${sql}`);
