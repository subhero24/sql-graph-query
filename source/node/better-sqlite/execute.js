...
export default function execute(db, query, parents) {
	let { type, sql, variables, relations } = query;

	if (parents == undefined) {
		if (sql) {
			let [attributeSql] = attributes(db, query);

			let entries = db.prepare(`${sql} RETURNING ${attributeSql}`).all(variables);
	...
	for (let relation of relations) {
		...
		if (relationIsJson) {
			...
			if (query.type == undefined) {
				...
				let key;
				let references = db.prepare(`PRAGMA foreign_key_list("${resource}")`).all();
			...
			let [attributeSql, shadowAttributes] = attributes(db, relation);

			let statement = db.prepare(`${scope} SELECT ${attributeSql} FROM temp ${sql}`);
			...
			try {
				for (let parent of parents) {
					...
					let children = statement.all(args);
					if (children.length) {
						patch(children, shadowAttributes);
						execute(db, relation, children);
					}
